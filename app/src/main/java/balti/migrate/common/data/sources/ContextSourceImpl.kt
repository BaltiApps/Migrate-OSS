...
class ContextSourceImpl(private val context: Context): ContextSource {
    override fun getProgressTitle(progressType: Progress.ProgressType): String {
        return when (progressType) {
            ...
            Progress.ProgressType.EXTERNAL_DATA_BACKUP -> context.getString(R.string.label_external_data_backup)
            ...
            Progress.ProgressType.APP_RESTORE -> context.getString(R.string.label_app_restore)
            Progress.ProgressType.EXTERNAL_DATA_RESTORE -> context.getString(R.string.label_external_data_restore)
            Progress.ProgressType.RESTORE_FINISHED -> context.getString(R.string.restore_finished)
            Progress.ProgressType.RESTORE_FINISHED_WITH_ERRORS -> context.getString(R.string.restore_finished_with_errors)
            Progress.ProgressType.RESTORE_CANCELLED -> context.getString(R.string.restore_cancelled)
